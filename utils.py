...
def load_chunks(data_dir):
    ...
    # with open(os.path.join(data_dir, "chunk_train_all.pkl"), "rb") as fr:
    #     chunk_train = pickle.load(fr)
    # with open(os.path.join(data_dir, "chunk_test_all.pkl"), "rb") as fr:
    #     chunk_test = pickle.load(fr)
...
def dump_scores(result_dir, hash_id, scores, converge):
    with open(os.path.join(result_dir, 'Eadro.txt'), 'a+') as fw:
